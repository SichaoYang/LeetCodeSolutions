...
This solution adopts the *latter*. Denote ``N := 2^ceil(log_2(n))``, such that ``N/2 < n = nums.length <= N``, we have ``tree[1:N-1]`` representing the non-leaf intervals, and ``tree[N:N+n-1]`` storing the leaf nodes, i.e., the individual integer indices.
...
At the end, every procrastinator still has to get the job done; every lazy update needs to be pushed down to the children.

#### Push updates down
Finally, we traverse the tree and let children inherit updates from parents recursively, as illustrated by the following example layer by layer:
```
               [0:5]
                 1
...
- Time complexity: ``O((n+m)*log(n))``, where ``n := nums.length`` and ``m := requests.length``.
  ...
  - While pushing updates down, we traverse the tree once, costing ``O(N) = O(n)``.
